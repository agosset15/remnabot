...
class UserDaoImpl(UserDao):
    ...
    async def get_by_remna_num_id(self, remna_num_id: int) -> Optional[UserDto]:
        stmt = (
            select(User)
            .join(Subscription, User.current_subscription_id == Subscription.id)
            .where(Subscription.user_remna_num_id == remna_num_id)
        )
        db_user = await self.session.scalar(stmt)

        if db_user:
            logger.debug(f"User with remna_num_id '{remna_num_id}' found in database")
            return self._convert_to_dto(db_user)

        logger.debug(f"User with remna_num_id '{remna_num_id}' not found")
        return None
