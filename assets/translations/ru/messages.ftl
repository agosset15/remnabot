...
msg-menu-invite =
    ...
    <b>📊 Статистика:</b>
    <blockquote>
    👀 Переходов по вашей ссылке: { $referral_count }
    💳 Покупок по вашей ссылке: { $referral_payments }
    💸 Всего заработано: { $referral_earned }
    </blockquote>

msg-invite-send =
    Рекомендую быстрый и дешевый VPN-сервис KaGo!

    При переходе по моей ссылке вы получите бонус 50 рублей на баланс!

    <a href="{ $referral_link }">Ссылка</a>

msg-menu-invited-users = Ваши приглашенные пользователи:
...
msg-importer-main =
    ...
    Также можно выполнить синхронизацию: бот проверит всех пользователей в панели.
    Если пользователя нет в базе бота, он будет создан и получит временную подписку.
    Если данные пользователя отличаются, они будут автоматически обновлены.
