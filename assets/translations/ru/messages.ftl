...
msg-profile-main =
    ...
    Здесь вы можете { $email ->
        [0] добавить
        *[HAS] изменить
    } почту, привязанную к аккаунту. На неё будут приходить уведомления о покупках и подключении.

msg-profile-email-input =
    <b>✉️ { $email_set ->
        [0] Добавление
        *[other] Изменение
    } почты</b>

    Отправьте новый email сообщением. На него придёт 6-значный код подтверждения.

msg-profile-email-otp =
    <b>🔐 Подтверждение почты</b>

    На <code>{ $pending_email }</code> отправлен 6-значный код. Введите его сообщением, чтобы завершить привязку.

    Код действует 10 минут. Если письмо не пришло — проверьте папку «Спам» или запросите новый код.
