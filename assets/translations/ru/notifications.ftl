...
ntf-profile =
    .email-otp-sent = ✅ <i>Код подтверждения отправлен на почту.</i>
    .email-otp-invalid = ❌ <i>Неверный код подтверждения.</i>
    .email-otp-expired = ❌ <i>Код истёк. Запросите новый.</i>
    .email-otp-cooldown = ⏳ <i>Подождите { $seconds } сек. перед повторной отправкой.</i>
    .email-invalid = ❌ <i>Неверный формат email.</i>
    .email-duplicate = ❌ <i>Эта почта уже используется другим пользователем.</i>
    .email-same = ℹ️ <i>Эта почта уже привязана к вашему аккаунту.</i>
    .email-send-failed = ❌ <i>Не удалось отправить письмо. Попробуйте позже.</i>
    .email-changed = ✅ <i>Почта успешно обновлена.</i>
...
email-connect-telegram =
    ...
    .message-html =
        { hdr-email-html }
        <div style="display:none;">Подключите Telegram-бота — управляйте подпиской в пару кликов.</div>
        <table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center" class="wrap" style="padding:36px 16px;">
        <table class="card" width="560" style="max-width:560px;width:100%;background:#fff;border:1px solid #DDE6F4;border-radius:16px;">
        <tr><td class="hd" align="center" style="background:#EFF6FF;border-bottom:1px solid #DBEAFE;padding:40px 32px 32px;border-radius:16px 16px 0 0;">
